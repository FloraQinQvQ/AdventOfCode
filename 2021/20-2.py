...
def enhancing(matrix, init_row, init_col, times=2):
    for _ in range(times):
        ...
        matrix = nd
    return len([key for key in matrix if matrix[key] == '#' and -times <= key[0] <= init_row + times - 1
                and -times <= key[1] <= init_col + times - 1])


print(enhancing(matrix, init_row, init_col, 50))
